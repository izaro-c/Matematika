...
const PORT = Number(process.env.MATEMATIKA_REAL_DIAGRAMS_E2E_PORT || 5178);
...
async function waitForServer(server: ChildProcess) {
  ...
  throw new Error('Vite no quedó disponible para la aceptación de diagramas reales.');
}
...
async function main() {
  ...
  try {
    ...
    console.log('✅ E2E: 4 páginas reales y editor visual de Pitágoras verificados.');
  } finally {
    server.kill('SIGTERM');
  }
}
