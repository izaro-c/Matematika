...
import corpus from '../../../fixtures/editor/real-mdx-corpus.json';
import {
  applyMutationPlan,
  parseEditorDocument,
  planBlockDeletion,
...
describe('Structural and lossless MDX engine', () => {
...
<ProofStep number={2} title="Dos">
  ...
  it('handles empty documents and documents without metadata export gracefully', () => {
    const emptyDoc = parseEditorDocument('');
    expect(emptyDoc.source).toBe('');
    expect(emptyDoc.metadata.status).toBe('missing');
    expect(emptyDoc.bodyBlocks).toHaveLength(0);

    const proseOnly = parseEditorDocument('# Título sin metadatos\n\nTexto libre.');
    expect(proseOnly.metadata.status).toBe('missing');
    expect(proseOnly.source).toContain('Título sin metadatos');
    expect(proseOnly.bodyBlocks.length).toBeGreaterThan(0);
  });
});
