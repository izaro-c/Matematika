...
describe('stepEmphasisAnimation', () => {
  ...
  it('adds pulse delta to the base emphasis value', () => {
    expect(pulsedPrimaryValue(3.2, STEP_EMPHASIS_STROKE_DELTA, 0)).toBeCloseTo(3.2, 5);
  ...
  it('pulses stroke width for primary emphasis on line-like elements', () => {
    expect(resolveStepEmphasisStrokeWidth(undefined, primaryState(0))).toBeCloseTo(3.2, 5);
  ...
  it('uses the configured highlight stroke width for secondary emphasis, like hover', () => {
    expect(resolveStepEmphasisStrokeWidth(undefined, secondaryState)).toBeCloseTo(3.6, 5);
    expect(resolveStepEmphasisStrokeWidth({ strokeWidth: 2.4, highlightStrokeWidth: 4.8 }, secondaryState)).toBeCloseTo(4.8, 5);
  });
