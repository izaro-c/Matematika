...
import { parseAttributes, type Block } from '@/fixed-pages/editor/session/parser';
...
function normalizeDiagramImportPath(source: string): string | undefined {
  ...
  if (withoutExtension.startsWith('@content/')) return `content/${withoutExtension.slice('@content/'.length)}.tsx`;
  const contentIndex = withoutExtension.indexOf('content/diagrams/');
  if (contentIndex >= 0) return `${withoutExtension.slice(contentIndex)}.tsx`;
