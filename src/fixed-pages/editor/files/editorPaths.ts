...
export function buildContentPath(folder: string, fileName: string): string {
  ...
  return `content/mdx/${normalizedFolder}/${normalizeContentId(stem)}${extension}`;
}
...
export function buildDiagramPath(category: string, fileName: string): string {
  const normalizedCategory = category
    ...
    .filter(Boolean)
    .join('/');
  return `content/diagrams/${normalizedCategory}/${normalizeContentId(fileName.replace(/\.(tsx|mdx)$/i, ''))}.tsx`;
}
