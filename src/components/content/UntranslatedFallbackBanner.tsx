...
export const UntranslatedFallbackBanner: React.FC<UntranslatedFallbackBannerProps> = ({ availableLangs, className = '' }) => {
  ...
  return (
    <div className={`w-full mb-6 p-4 bg-ocre/10 dark:bg-ocre/15 border border-ocre/30 text-carbon dark:text-papiro text-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in fade-in duration-200 ${className}`.trim()}>
        ...
        {availableLangs.map((langCode) => {
          ...
          return (
            <button
              ...
              className="px-3 py-1 text-xs font-medium border border-ocre/30 text-carbon hover:text-lienzo hover:bg-ocre transition-colors duration-150"
            >
              {langInfo.name}
            </button>
          );
