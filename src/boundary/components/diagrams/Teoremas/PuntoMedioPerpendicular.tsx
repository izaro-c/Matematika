...
export const PuntoMedioPerpendicular = () => {
  ...
  useEffect(() => {
    ...
    const sq1 = board.create('point', [() => { const d = segDir(); return M.X() + d.x*sqSize; }, () => { const d = segDir(); return M.Y() + d.y*sqSize; }], { visible: false });
    const sq2 = board.create('point', [() => { const d = segDir(); const p = perpDir(); return M.X() + d.x*sqSize + p.x*sqSize; }, () => { const d = segDir(); const p = perpDir(); return M.Y() + d.y*sqSize + p.y*sqSize; }], { visible: false });
    const sq3 = board.create('point', [() => { const p = perpDir(); return M.X() + p.x*sqSize; }, () => { const p = perpDir(); return M.Y() + p.y*sqSize; }], { visible: false });
    ...
    const mkTick = (p: any, q: any) => {
      ...
      const t0 = board.create('point', [() => { const dx = q.X()-p.X(), dy = q.Y()-p.Y(); const len = Math.hypot(dx, dy)||1; return mA.X()+dy/len*.25; }, () => { const dx = q.X()-p.X(), dy = q.Y()-p.Y(); const len = Math.hypot(dx, dy)||1; return mA.Y()-dx/len*.25; }], { visible: false });
      const t1 = board.create('point', [() => { const dx = q.X()-p.X(), dy = q.Y()-p.Y(); const len = Math.hypot(dx, dy)||1; return mA.X()-dy/len*.25; }, () => { const dx = q.X()-p.X(), dy = q.Y()-p.Y(); const len = Math.hypot(dx, dy)||1; return mA.Y()+dx/len*.25; }], { visible: false });
    ...
    const infoText = board.create('text', [-5.5, 5.5, () => {
      ...
      return `<div style="font-family: var(--font-serif); color: ${getCSSVar('--theme-carbon')}; line-height:1.3;">
        ...
        <strong style="color:${Math.abs(dPA-dPB)<0.01 ? getCSSVar('--theme-musgo') : getCSSVar('--theme-granada')};">${Math.abs(dPA-dPB)<0.01 ? 'PA \u2261 PB' : 'PA \u2260 PB'}</strong>
      </div>`;
    ...
    return () => { observer.disconnect(); JXG.JSXGraph.freeBoard(board); elementsRef.current = {}; };
  }, []);
