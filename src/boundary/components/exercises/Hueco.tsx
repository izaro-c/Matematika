...
export const Hueco: React.FC<HuecoProps> = ({
  ...
  // Registrarse en el contexto al montar
  ...
  // Renderizado Inline (sin pregunta)
  if (!pregunta) {
    if (isDone) {
      return (
    ...
    return (
      <span className="inline-flex flex-col items-start relative group">
        ...
        <span className="inline-flex items-center gap-2">
          ...
          />
          {input.trim() && (
            <button onClick={check} className="text-[10px] text-terracota opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-widest font-sans">
              ↵
            </button>
          )}
        </span>
