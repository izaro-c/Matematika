...
const indexPath = path.resolve('./src/features/editor/index.tsx');
let indexContent = fs.readFileSync(indexPath, 'utf8');

const useEditorStateContent = `import { useState, useRef, useCallback, useEffect } from 'react';
...
export const useEditorState = () => {
  ...
  const saveDraft = useCallback(async (meta: any, imp: string, bod: string, file: string) => {
  ...
  const scheduleDraft = (newMeta: any, newImp: string, newBod: string) => {
  ...
  const getPreviewUrl = () => {
    ...
    if (currentFile.startsWith('content/lessons')) return \`/\${metadata.id}\`;
...
fs.writeFileSync(path.resolve('./src/features/editor/hooks/useEditorState.ts'), useEditorStateContent, 'utf8');
console.log('Created useEditorState.ts');
